...
本分支将默认 Embedding Profile 从测试用 Hash 改为 `sentence-transformers:BAAI/bge-m3:1024`，并把 Knowledge Graph Builder/Extractor Profile 纳入不可变 `version_id` 计算。因此旧 Hash 版本不会被误认为新语义版本。
